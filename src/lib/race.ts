...
import { evalHand, computeSessionStats, pLandInDraws, landCountFromDeck } from './metrics';

export function runDeckTrials(deck: Card[], n = 20): DeckTrialResult {
  const deckLands = landCountFromDeck(deck);
  const samples: { landCount: number; wasKept: boolean; keep: string; t3: number }[] = [];
...
export function generateVerdict(a: DeckTrialResult, b: DeckTrialResult): string[] {
  const lines: string[] = [];
